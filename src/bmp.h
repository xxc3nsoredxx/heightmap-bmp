...
typedef struct dib_h {
    int size_dib_h; // Size of DIB header (40) - 4 bytes
    ...
    int bi_rgb; // Pixel compression (0: none) - 4 bytes
...
dib_h create_dib_header (FILE *);
pixel** create_pixel_array (FILE *, int, int);
void write_data (FILE *, bmp_file);

void flip_endian (int *);
